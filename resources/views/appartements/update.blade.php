...
/* ══ BADGE MODIFICATION ══ */
.edit-badge{display:inline-flex;align-items:center;gap:6px;background:var(--gr-lt);color:var(--gr-dk);border:1.5px solid var(--gr-b);border-radius:20px;padding:5px 14px;font-size:11px;font-weight:700;letter-spacing:.04em;text-transform:uppercase;margin-bottom:14px;}
...
/* ══ FORM FIELDS ══ */
.fg{margin-bottom:0;}.fg + .fg{margin-top:14px;}
.fg label{display:block;font-size:12px;font-weight:600;color:var(--ink2);margin-bottom:5px;}
.fg input[type="text"],.fg input[type="number"],.fg input[type="date"],.fg select,.fg textarea{
    display:block;width:100%;padding:10px 14px;border-radius:var(--r);
    border:1.5px solid var(--bord);background:#FAFBFC;
    font-size:13px;font-family:sans-serif;color:var(--ink);outline:none;-webkit-appearance:none;
...
.fi{position:relative;}.fi input{padding-right:52px;}
...
.yn-b{padding:8px 20px;border-radius:28px;border:1.5px solid var(--bord);background:var(--white);font-size:12.5px;font-weight:700;color:var(--ink2);cursor:pointer;transition:all .2s;font-family:sans-serif;}
...
.geo-status{font-size:11.5px;font-weight:600;display:none;align-items:center;gap:6px;margin-top:7px;}
.geo-status.ok{color:var(--gr);}
.geo-status.err{color:var(--red);}
.geo-status.loading{color:var(--blue);}

/* ══ PHOTOS EXISTANTES ══ */
.ex-grid{display:flex;gap:8px;flex-wrap:wrap;margin-bottom:14px;}
.ex-ph{width:82px;height:82px;border-radius:9px;overflow:hidden;position:relative;border:1.5px solid var(--gr-b);}
.ex-ph img{width:100%;height:100%;object-fit:cover;display:block;}
.ex-ph-lbl{position:absolute;bottom:0;left:0;right:0;background:rgba(21,128,61,.75);color:#fff;font-size:8px;font-weight:700;text-align:center;padding:3px;letter-spacing:.04em;}
.ep-del{position:absolute;top:3px;right:3px;width:18px;height:18px;border-radius:50%;background:rgba(220,38,38,.8);color:#fff;border:none;font-size:9px;cursor:pointer;display:flex;align-items:center;justify-content:center;}
.ex-ph.deleted{opacity:.35;}
.ex-ph.deleted .ex-ph-lbl{background:rgba(220,38,38,.75);}
...
.vid-existing-box{border:1.5px solid var(--gr-b);border-radius:var(--r-lg);padding:12px 14px;margin-bottom:12px;background:var(--gr-lt);display:flex;align-items:center;gap:12px;}
.vid-existing-box video{width:100px;height:68px;object-fit:cover;border-radius:7px;flex-shrink:0;}
.vex-info{flex:1;}
.vex-name{font-size:12px;font-weight:700;color:var(--ink);margin-bottom:3px;}
.vex-sub{font-size:11px;color:var(--gr);}
.btn-vex-del{background:none;border:1.5px solid rgba(220,38,38,.2);color:var(--red);font-size:11px;font-weight:700;cursor:pointer;padding:4px 10px;border-radius:7px;font-family:sans-serif;}
.btn-vex-del:hover{background:var(--red-lt);}
...
/* Responsive */
@media(max-width:991px){.sl-side{display:none;}.sl-main{padding:18px 14px 56px;max-width:100%;}.hero-carousel{height:180px;}.sp-title{font-size:19px;}}
@media(max-width:767px){.sl-main{padding:14px 11px 46px;}.hero-carousel{height:140px;}.oc{min-width:65px;max-width:none;flex:1 1 calc(50% - 7px);}.eq{min-width:calc(50% - 5px);flex:1 1 calc(50% - 5px);}.surface-grid,.pieces-grid{grid-template-columns:repeat(2,1fr);}.compteur-grid{grid-template-columns:1fr;}.s-box{padding:14px 12px;}.sl-nav{flex-direction:column-reverse;gap:8px;}.btn-next,.btn-prev{width:100%;justify-content:center;}.geo-coords,.row-gap{flex-direction:column;gap:10px;}.geo-coords .fg,.row-gap>.fg{min-width:100%;}}
@media(max-width:480px){.hero-carousel{height:110px;}.oc{flex:1 1 calc(50% - 5px);padding:10px 4px 8px;}.surface-grid,.pieces-grid{grid-template-columns:repeat(2,1fr);}}
</style>
...
<div class="sl-wrap">
    ...
    {{-- SIDEBAR --}}
    <aside class="sl-side">
        <div class="sl-side-ttl">Modification annonce</div>
        <ul class="sl-list" id="slList">
            ...
            <li class="sl-item" data-step="5"><div class="si-dot"><span class="si-num">5</span></div><div class="si-txt"><span class="si-name">Description</span><span class="si-st">À compléter</span></div></li>
    ...
    <main class="sl-main">
        <form id="form" method="POST"
              action="{{ route('appartement.update', $appartement->id) }}"
            ...
            <div class="sl-pane active" id="pane1">
                <div class="edit-badge"><i class="fa fa-pencil"></i> Modification de l'annonce</div>
                ...
                <div class="s-box">
                    ...
                    <div class="oc-row" id="typeGrid">
                        <label class="oc {{ $appartement->type=='Maison'?'s':'' }}" data-v="Maison">
                            <input type="radio" name="type" value="Maison" {{ $appartement->type=='Maison'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-home"></i><span>Maison</span>
                        </label>
                        <label class="oc {{ $appartement->type=='Appartement'?'s':'' }}" data-v="Appartement">
                            <input type="radio" name="type" value="Appartement" {{ $appartement->type=='Appartement'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-building"></i><span>Appartement</span>
                        </label>
                        <label class="oc {{ $appartement->type=='Terrain'?'s':'' }}" data-v="Terrain">
                            <input type="radio" name="type" value="Terrain" {{ $appartement->type=='Terrain'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-map"></i><span>Terrain</span>
                        </label>
                        <label class="oc {{ $appartement->type=='Bureaux'?'s':'' }}" data-v="Bureaux">
                            <input type="radio" name="type" value="Bureaux" {{ $appartement->type=='Bureaux'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-briefcase"></i><span>Bureau</span>
                        </label>
                        <label class="oc {{ $appartement->type=='Boutique'?'s':'' }}" data-v="Boutique">
                            <input type="radio" name="type" value="Boutique" {{ $appartement->type=='Boutique'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-shopping-bag"></i><span>Boutique</span>
                        </label>
                    </div>
                </div>
                ...
                <div class="s-box">
                    ...
                    <div class="oc-row" id="catGrid" style="max-width:280px;">
                        <label class="oc {{ $appartement->categorie=='louer'?'s':'' }}" data-v="louer">
                            <input type="radio" name="categorie" value="louer" {{ $appartement->categorie=='louer'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-key"></i><span>Louer</span>
                        </label>
                        <label class="oc {{ $appartement->categorie=='vendre'?'s':'' }}" data-v="vendre">
                            <input type="radio" name="categorie" value="vendre" {{ $appartement->categorie=='vendre'?'checked':'' }}>
                            <div class="oc-tick"></div><i class="fa fa-tag"></i><span>Vendre</span>
                        </label>
                    </div>
                </div>
            ...
            <div class="sl-pane" id="pane2">
                ...
                <div class="s-box">
                    ...
                    <div class="fg" style="position:relative;">
                        ...
                        <input type="text" id="quartierInput" name="quartier"
                               value="{{ $appartement->quartier }}"
                               placeholder="Ex: Akpakpa, Fidjrossè, Godomey…" autocomplete="off">
                ...
                <div class="s-box">
                    ...
                    <div class="geo-box" id="geoBox" style="{{ $appartement->latitude ? 'display:block;' : 'display:none;' }}">
                        <div class="geo-box-title"><i class="fa fa-map-pin"></i> Coordonnées {{ $appartement->latitude ? 'enregistrées' : 'détectées' }}</div>
                        <div class="geo-coords">
                            <div class="fg"><label>Latitude</label><input type="text" name="latitude" id="geoLat" value="{{ $appartement->latitude }}" readonly placeholder="—"></div>
                            <div class="fg"><label>Longitude</label><input type="text" name="longitude" id="geoLng" value="{{ $appartement->longitude }}" readonly placeholder="—"></div>
                        </div>
            ...
            <div class="sl-pane" id="pane3">
                ...
                <div class="s-box type-block" data-types="Maison,Appartement">
                    ...
                    <div class="pieces-grid">
                        <div class="piece-card {{ intval($appartement->nombreSalon)>0?'has-value':'' }}" id="pcSalon"><div class="piece-card-ico"><i class="fa fa-tv"></i></div><div class="piece-card-lbl">Salon(s)</div><div class="ctr"><button type="button" class="cb" data-t="nombreSalon" data-a="-">−</button><input type="number" name="nombreSalon" id="nombreSalon" class="cv" value="{{ $appartement->nombreSalon ?? 0 }}" min="0" readonly><button type="button" class="cb" data-t="nombreSalon" data-a="+">+</button></div></div>
                        <div class="piece-card {{ intval($appartement->nombreCuisine)>0?'has-value':'' }}" id="pcCuisine"><div class="piece-card-ico"><i class="fa fa-cutlery"></i></div><div class="piece-card-lbl">Cuisine(s)</div><div class="ctr"><button type="button" class="cb" data-t="nombreCuisine" data-a="-">−</button><input type="number" name="nombreCuisine" id="nombreCuisine" class="cv" value="{{ $appartement->nombreCuisine ?? 0 }}" min="0" readonly><button type="button" class="cb" data-t="nombreCuisine" data-a="+">+</button></div></div>
                        <div class="piece-card {{ intval($appartement->nombreChambre)>0?'has-value':'' }}" id="pcChambre"><div class="piece-card-ico"><i class="fa fa-bed"></i></div><div class="piece-card-lbl">Chambre(s)</div><div class="ctr"><button type="button" class="cb" data-t="nombreChambre" data-a="-">−</button><input type="number" name="nombreChambre" id="nombreChambre" class="cv" value="{{ $appartement->nombreChambre ?? 0 }}" min="0" readonly><button type="button" class="cb" data-t="nombreChambre" data-a="+">+</button></div></div>
                        <div class="piece-card {{ intval($appartement->nombreSalleBain)>0?'has-value':'' }}" id="pcSdb"><div class="piece-card-ico"><i class="fa fa-shower"></i></div><div class="piece-card-lbl">Salle(s) de bain</div><div class="ctr"><button type="button" class="cb" data-t="nombreSalleBain" data-a="-">−</button><input type="number" name="nombreSalleBain" id="nombreSalleBain" class="cv" value="{{ $appartement->nombreSalleBain ?? 0 }}" min="0" readonly><button type="button" class="cb" data-t="nombreSalleBain" data-a="+">+</button></div></div>
                    </div>
                    <div class="pieces-total-badge"><i class="fa fa-calculator"></i> Total : <strong id="piecesTotalDisplay">{{ $appartement->nombrePieces ?? 0 }}</strong> pièce(s)</div>
                    <input type="hidden" name="nombrePieces" id="nombrePieces" value="{{ $appartement->nombrePieces ?? 0 }}">
                </div>

                <div class="s-box type-block" data-types="Maison,Appartement">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-th-large"></i></div> Surface par pièce</div>
                    <div class="surface-grid">
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-home"></i> Salon</div><input type="number" name="surface_salon" id="surfSalon" value="{{ $appartement->surface_salon ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">m²</div></div>
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-cutlery"></i> Cuisine</div><input type="number" name="surface_cuisine" id="surfCuisine" value="{{ $appartement->surface_cuisine ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">m²</div></div>
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-bed"></i> Chambre</div><input type="number" name="surface_chambre" id="surfChambre" value="{{ $appartement->surface_chambre ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">m²/ch.</div></div>
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-shower"></i> Salle de bain</div><input type="number" name="surface_sdb" id="surfSdb" value="{{ $appartement->surface_sdb ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">m²</div></div>
                        <div class="surf-card"><div class="surf-card-lbl" style="color:var(--gr-dk);"><i class="fa fa-calculator"></i> Totale</div><input type="number" name="surface" id="surface" value="{{ $appartement->surface ?? '' }}" placeholder="0" min="1" style="color:var(--gr);"><div class="surf-card-unit" style="color:var(--gr);">m² (auto)</div></div>
                    </div>
                </div>
                ...
                <div class="s-box type-block" data-types="Maison,Appartement,Bureaux,Boutique">
                    ...
                    <div class="fg" style="max-width:200px;"><label>Disponible à partir de</label><input type="date" name="disponible_date" id="dispoDate" value="{{ $appartement->disponible_date ? \Carbon\Carbon::parse($appartement->disponible_date)->format('Y-m-d') : '' }}"></div>
                </div>

                <div class="s-box type-block" data-types="Maison,Appartement">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-check-circle-o"></i></div> Options résidentielles</div>
                    @php
                        $collocation = $appartement->collocation ?? 'Non';
                        $packing     = $appartement->packing ?? 'Non';
                        $sanitaire   = $appartement->sanitaire ?? 'Non';
                        $proprioVit  = $appartement->proprio_vit ?? 'Non';
                    @endphp
                    <div class="fg"><label>Colocation possible ?</label><div class="yn"><button type="button" class="yn-b {{ $collocation=='Oui'?'s':'' }}" data-v="Oui" data-t="collocation">Oui</button><button type="button" class="yn-b {{ $collocation!='Oui'?'s':'' }}" data-v="Non" data-t="collocation">Non</button><input type="hidden" name="collocation" id="collocation" value="{{ $collocation }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Parking / Stationnement ?</label><div class="yn"><button type="button" class="yn-b {{ $packing=='Oui'?'s':'' }}" data-v="Oui" data-t="packing">Oui</button><button type="button" class="yn-b {{ $packing!='Oui'?'s':'' }}" data-v="Non" data-t="packing">Non</button><input type="hidden" name="packing" id="packing" value="{{ $packing }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Sanitaire (WC) ?</label><div class="yn"><button type="button" class="yn-b {{ $sanitaire=='Oui'?'s':'' }}" data-v="Oui" data-t="sanitaire">Oui</button><button type="button" class="yn-b {{ $sanitaire!='Oui'?'s':'' }}" data-v="Non" data-t="sanitaire">Non</button><input type="hidden" name="sanitaire" id="sanitaire" value="{{ $sanitaire }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Le propriétaire vit-il dans la maison ?</label><div class="yn"><button type="button" class="yn-b {{ $proprioVit=='Oui'?'s':'' }}" data-v="Oui" data-t="proprio_vit">Oui</button><button type="button" class="yn-b {{ $proprioVit!='Oui'?'s':'' }}" data-v="Non" data-t="proprio_vit">Non</button><input type="hidden" name="proprio_vit" id="proprio_vit" value="{{ $proprioVit }}"></div></div>
                    <div class="section-sep"></div>
                    <div class="sub-section-title"><i class="fa fa-bolt" style="color:var(--gr);margin-right:4px;"></i> Compteurs — Eau &amp; Électricité</div>
                    @php
                        $cptEau  = $appartement->compteur_eau  ?? '';
                        $cptElec = $appartement->compteur_elec ?? '';
                    @endphp
                    <div class="fg" style="margin-bottom:10px;"><label>Compteur eau</label>
                        <div class="compteur-grid" id="compteurEauGrid">
                            <label class="compt-opt {{ $cptEau=='Oui'?'s':'' }}" data-t="compteur_eau" data-v="Oui"><input type="radio" name="compteur_eau" value="Oui" {{ $cptEau=='Oui'?'checked':'' }}><div class="compt-opt-ico"><i class="fa fa-tint"></i></div><div class="compt-opt-txt"><span class="compt-opt-name">Oui</span><span class="compt-opt-sub">Compteur personnel</span></div><div class="compt-tick"></div></label>
                            <label class="compt-opt {{ $cptEau=='Non'?'s':'' }}" data-t="compteur_eau" data-v="Non"><input type="radio" name="compteur_eau" value="Non" {{ $cptEau=='Non'?'checked':'' }}><div class="compt-opt-ico"><i class="fa fa-times"></i></div><div class="compt-opt-txt"><span class="compt-opt-name">Non</span><span class="compt-opt-sub">Pas de compteur</span></div><div class="compt-tick"></div></label>
                        </div>
                        <input type="hidden" name="compteur_eau_val" id="compteurEauVal" value="{{ $cptEau }}">
                    </div>
                    <div class="fg"><label>Compteur électricité</label>
                        <div class="compteur-grid" id="compteurElecGrid">
                            <label class="compt-opt {{ $cptElec=='Standard'?'s':'' }}" data-t="compteur_elec" data-v="Standard"><input type="radio" name="compteur_elec" value="Standard" {{ $cptElec=='Standard'?'checked':'' }}><div class="compt-opt-ico"><i class="fa fa-bolt"></i></div><div class="compt-opt-txt"><span class="compt-opt-name">Standard</span><span class="compt-opt-sub">Classique</span></div><div class="compt-tick"></div></label>
                            <label class="compt-opt {{ $cptElec=='Carte prépayée'?'s':'' }}" data-t="compteur_elec" data-v="Carte prépayée"><input type="radio" name="compteur_elec" value="Carte prépayée" {{ $cptElec=='Carte prépayée'?'checked':'' }}><div class="compt-opt-ico"><i class="fa fa-credit-card"></i></div><div class="compt-opt-txt"><span class="compt-opt-name">Prépayé</span><span class="compt-opt-sub">Rechargeable</span></div><div class="compt-tick"></div></label>
                            <label class="compt-opt {{ $cptElec=='Non'?'s':'' }}" data-t="compteur_elec" data-v="Non"><input type="radio" name="compteur_elec" value="Non" {{ $cptElec=='Non'?'checked':'' }}><div class="compt-opt-ico"><i class="fa fa-times"></i></div><div class="compt-opt-txt"><span class="compt-opt-name">Non</span><span class="compt-opt-sub">Pas de compteur</span></div><div class="compt-tick"></div></label>
                        </div>
                        <input type="hidden" name="compteur_elec_val" id="compteurElecVal" value="{{ $cptElec }}">
                    </div>
                </div>
                ...
                <div class="s-box type-block" data-types="Maison,Appartement">
                    ...
                    <div class="eq-row">
                        <label class="eq {{ $appartement->cuisine=='Oui'?'s':'' }}"><input type="checkbox" name="cuisine_eq" value="Oui" {{ $appartement->cuisine=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-cutlery"></i><span>Cuisine équipée</span></label>
                        <label class="eq {{ $appartement->clime=='Climatiseur'?'s':'' }}"><input type="checkbox" name="clime_c" value="Climatiseur" {{ $appartement->clime=='Climatiseur'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-snowflake-o"></i><span>Climatiseur</span></label>
                        <label class="eq"><input type="checkbox" name="brasseur" value="Oui"><div class="eq-tick"></div><i class="fa fa-circle-o-notch"></i><span>Brasseur d'air</span></label>
                        <label class="eq {{ $appartement->wifi=='Wifi'?'s':'' }}"><input type="checkbox" name="wifi_c" value="Wifi" {{ $appartement->wifi=='Wifi'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-wifi"></i><span>Wifi</span></label>
                        <label class="eq {{ $appartement->securite=='Oui'?'s':'' }}"><input type="checkbox" name="securite_c" value="Oui" {{ $appartement->securite=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-shield"></i><span>Sécurité</span></label>
                        <label class="eq {{ $appartement->terasse=='Oui'?'s':'' }}"><input type="checkbox" name="terasse_c" value="Oui" {{ $appartement->terasse=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-sun-o"></i><span>Terrasse</span></label>
                        <label class="eq {{ ($appartement->entretien??'Non inclus')!='Non inclus'?'s':'' }}"><input type="checkbox" name="entretien_c" value="Oui" {{ ($appartement->entretien??'Non inclus')!='Non inclus'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-magic"></i><span>Entretien inclus</span></label>
                    </div>
                </div>
                ...
                <div class="s-box type-block" data-types="Terrain">
                    ...
                    <div class="surface-grid">
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-arrows-h"></i> Largeur</div><input type="number" name="terrain_largeur" id="terrainLargeur" value="{{ $appartement->terrain_largeur ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">mètres</div></div>
                        <div class="surf-card"><div class="surf-card-lbl"><i class="fa fa-arrows-v"></i> Longueur</div><input type="number" name="terrain_longueur" id="terrainLongueur" value="{{ $appartement->terrain_longueur ?? '' }}" placeholder="0" min="0"><div class="surf-card-unit">mètres</div></div>
                        <div class="surf-card"><div class="surf-card-lbl" style="color:var(--gr-dk);"><i class="fa fa-calculator"></i> Superficie</div><input type="number" name="surface" id="surfaceTerrain" value="{{ $appartement->type=='Terrain'?($appartement->surface??''):'' }}" placeholder="0" min="1" style="color:var(--gr);"><div class="surf-card-unit" style="color:var(--gr);">m² (auto)</div></div>
                    </div>
                </div>
                ...
                <div class="s-box type-block" data-types="Terrain">
                    ...
                    <div class="fg">
                        ...
                        <select name="titre_foncier">
                            <option value="">Sélectionner</option>
                            <option value="TF" {{ ($appartement->titre_foncier??'')=='TF'?'selected':'' }}>Titre Foncier (TF)</option>
                            <option value="LR" {{ ($appartement->titre_foncier??'')=='LR'?'selected':'' }}>Lettre de Réclamation (LR)</option>
                            <option value="Convention" {{ ($appartement->titre_foncier??'')=='Convention'?'selected':'' }}>Convention</option>
                            <option value="ACD" {{ ($appartement->titre_foncier??'')=='ACD'?'selected':'' }}>Acte de Cession Définitive (ACD)</option>
                            <option value="Reçu" {{ ($appartement->titre_foncier??'')=='Reçu'?'selected':'' }}>Reçu de vente</option>
                            <option value="Autre" {{ ($appartement->titre_foncier??'')=='Autre'?'selected':'' }}>Autre</option>
                        </select>
                    </div>
                    @php $cloture=$appartement->cloture??'Non'; $viabilise=$appartement->viabilise??'Non'; $constructible=$appartement->constructible??'Oui'; @endphp
                    <div class="fg" style="margin-top:12px;"><label>Terrain clôturé ?</label><div class="yn"><button type="button" class="yn-b {{ $cloture=='Oui'?'s':'' }}" data-v="Oui" data-t="cloture">Oui</button><button type="button" class="yn-b {{ $cloture!='Oui'?'s':'' }}" data-v="Non" data-t="cloture">Non</button><input type="hidden" name="cloture" id="cloture" value="{{ $cloture }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Viabilisé (eau, électricité) ?</label><div class="yn"><button type="button" class="yn-b {{ $viabilise=='Oui'?'s':'' }}" data-v="Oui" data-t="viabilise">Oui</button><button type="button" class="yn-b {{ $viabilise!='Oui'?'s':'' }}" data-v="Non" data-t="viabilise">Non</button><input type="hidden" name="viabilise" id="viabilise" value="{{ $viabilise }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Constructible ?</label><div class="yn"><button type="button" class="yn-b {{ $constructible=='Oui'?'s':'' }}" data-v="Oui" data-t="constructible">Oui</button><button type="button" class="yn-b {{ $constructible!='Oui'?'s':'' }}" data-v="Non" data-t="constructible">Non</button><input type="hidden" name="constructible" id="constructible" value="{{ $constructible }}"></div></div>
                </div>
                ...
                <div class="s-box type-block" data-types="Bureaux">
                    ...
                    <div class="row-gap">
                        <div class="fg"><label>Surface totale (m²) <span style="color:var(--red)">*</span></label><div class="fi"><input type="number" name="surface_bureau" id="surfaceBureau" value="{{ $appartement->surface ?? '' }}" placeholder="Ex: 60" min="1"><span class="fi-sfx">m²</span></div></div>
                        <div class="fg"><label>Nombre de pièces / postes</label><div class="ctr"><button type="button" class="cb" data-t="nombrePiecesBureaux" data-a="-">−</button><input type="number" name="nombrePiecesBureaux" id="nombrePiecesBureaux" class="cv" value="{{ $appartement->nombrePieces ?? 1 }}" min="1" readonly><button type="button" class="cb" data-t="nombrePiecesBureaux" data-a="+">+</button></div></div>
                    </div>
                </div>

                <div class="s-box type-block" data-types="Bureaux">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-briefcase"></i></div> Options bureau</div>
                    @php $packB=$appartement->packing??'Non'; $sanB=$appartement->sanitaire??'Non'; @endphp
                    <div class="fg"><label>Parking ?</label><div class="yn"><button type="button" class="yn-b {{ $packB=='Oui'?'s':'' }}" data-v="Oui" data-t="packing_bureaux">Oui</button><button type="button" class="yn-b {{ $packB!='Oui'?'s':'' }}" data-v="Non" data-t="packing_bureaux">Non</button><input type="hidden" name="packing_bureaux" id="packing_bureaux" value="{{ $packB }}"></div></div>
                    <div class="fg" style="margin-top:12px;"><label>Sanitaire (WC) ?</label><div class="yn"><button type="button" class="yn-b {{ $sanB=='Oui'?'s':'' }}" data-v="Oui" data-t="sanitaire_bureaux">Oui</button><button type="button" class="yn-b {{ $sanB!='Oui'?'s':'' }}" data-v="Non" data-t="sanitaire_bureaux">Non</button><input type="hidden" name="sanitaire_bureaux" id="sanitaire_bureaux" value="{{ $sanB }}"></div></div>
                </div>

                <div class="s-box type-block" data-types="Bureaux">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-plug"></i></div> Équipements bureau</div>
                    <div class="eq-row">
                        <label class="eq {{ $appartement->clime=='Climatiseur'?'s':'' }}"><input type="checkbox" name="clime_b" value="Climatiseur" {{ $appartement->clime=='Climatiseur'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-snowflake-o"></i><span>Climatiseur</span></label>
                        <label class="eq"><input type="checkbox" name="brasseur_b" value="Oui"><div class="eq-tick"></div><i class="fa fa-circle-o-notch"></i><span>Brasseur d'air</span></label>
                        <label class="eq {{ $appartement->wifi=='Wifi'?'s':'' }}"><input type="checkbox" name="wifi_b" value="Wifi" {{ $appartement->wifi=='Wifi'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-wifi"></i><span>Wifi</span></label>
                        <label class="eq {{ $appartement->securite=='Oui'?'s':'' }}"><input type="checkbox" name="securite_b" value="Oui" {{ $appartement->securite=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-shield"></i><span>Sécurité</span></label>
                        <label class="eq {{ $appartement->packing=='Oui'?'s':'' }}"><input type="checkbox" name="parking_b" value="Oui" {{ $appartement->packing=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-car"></i><span>Parking</span></label>
                        <label class="eq"><input type="checkbox" name="salle_conf" value="Oui"><div class="eq-tick"></div><i class="fa fa-users"></i><span>Salle de conf.</span></label>
                    </div>
                </div>
                ...
                <div class="s-box type-block" data-types="Boutique">
                    ...
                    <div class="fg" style="max-width:200px;"><label>Surface (m²) <span style="color:var(--red)">*</span></label><div class="fi"><input type="number" name="surface_boutique" id="surfaceBoutique" value="{{ $appartement->surface ?? '' }}" placeholder="Ex: 30" min="1"><span class="fi-sfx">m²</span></div></div>
                </div>

                <div class="s-box type-block" data-types="Boutique">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-plug"></i></div> Équipements boutique</div>
                    <div class="eq-row">
                        <label class="eq {{ $appartement->clime=='Climatiseur'?'s':'' }}"><input type="checkbox" name="clime_bt" value="Climatiseur" {{ $appartement->clime=='Climatiseur'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-snowflake-o"></i><span>Climatiseur</span></label>
                        <label class="eq"><input type="checkbox" name="brasseur_bt" value="Oui"><div class="eq-tick"></div><i class="fa fa-circle-o-notch"></i><span>Brasseur d'air</span></label>
                        <label class="eq {{ $appartement->securite=='Oui'?'s':'' }}"><input type="checkbox" name="securite_bt" value="Oui" {{ $appartement->securite=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-shield"></i><span>Sécurité</span></label>
                        <label class="eq {{ $appartement->packing=='Oui'?'s':'' }}"><input type="checkbox" name="parking_bt" value="Oui" {{ $appartement->packing=='Oui'?'checked':'' }}><div class="eq-tick"></div><i class="fa fa-car"></i><span>Parking</span></label>
                        <label class="eq"><input type="checkbox" name="vitrine_bt" value="Oui"><div class="eq-tick"></div><i class="fa fa-eye"></i><span>Vitrine</span></label>
                    </div>
                </div>

                <input type="hidden" name="surface_totale" id="surfaceTotaleHidden" value="{{ $appartement->surface ?? 0 }}">
                <input type="hidden" name="clime" id="hClime" value="{{ $appartement->clime ?? 'Non' }}">
                <input type="hidden" name="wifi" id="hWifi" value="{{ $appartement->wifi ?? 'Non' }}">
                <input type="hidden" name="securite" id="hSecurite" value="{{ $appartement->securite ?? 'Non' }}">
                <input type="hidden" name="terasse" id="hTerasse" value="{{ $appartement->terasse ?? 'Non' }}">
                <input type="hidden" name="entretien" id="hEntretien" value="{{ $appartement->entretien ?? 'Non inclus' }}">
                <input type="hidden" name="cuisine" id="hCuisine" value="{{ $appartement->cuisine ?? 'Non' }}">
                <input type="hidden" name="nombreChambreH" id="hChambre" value="{{ $appartement->nombreChambre ?? 0 }}">
                <input type="hidden" name="nombreSalonH" id="hSalon" value="{{ $appartement->nombreSalon ?? 0 }}">
                <input type="hidden" name="nombreCuisineH" id="hCuisineCount" value="{{ $appartement->nombreCuisine ?? 0 }}">
            </div>
            ...
            <div class="sl-pane" id="pane4">
                ...
                {{-- PHOTOS EXISTANTES --}}
                @php $existingImgs = array_values(array_filter(explode('|', $appartement->images ?? ''))); @endphp
                @if(count($existingImgs) > 0)
                <div class="s-box">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-images"></i></div> Photos actuelles <span style="font-size:10px;font-weight:400;color:var(--ink3);margin-left:4px;">— cliquez <i class="fa fa-times"></i> pour supprimer</span></div>
                    <div class="ex-grid" id="existingPhotos">
                        @foreach($existingImgs as $idx => $img)
                        <div class="ex-ph" id="exph-{{ $idx }}" data-img="{{ $img }}">
                            <img src="{{ URL::to($img) }}" alt="Photo {{ $idx+1 }}">
                            <div class="ex-ph-lbl">Photo {{ $idx+1 }}</div>
                            <button type="button" class="ep-del" onclick="toggleExisting({{ $idx }})"><i class="fa fa-times"></i></button>
                        </div>
                        @endforeach
                    </div>
                    <input type="hidden" name="images_to_keep" id="imagesToKeep" value="{{ $appartement->images }}">
                </div>
                @endif

                {{-- NOUVELLES PHOTOS --}}
                <div class="s-box">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-camera"></i></div> {{ count($existingImgs) > 0 ? 'Ajouter de nouvelles photos' : 'Photos' }} <span style="color:var(--red);margin-left:3px;font-size:10px;">{{ count($existingImgs) == 0 ? 'obligatoire' : '' }}</span></div>
                    <div class="upz" id="upz"><i class="fa fa-cloud-upload"></i><p>Cliquez ou glissez vos photos ici</p><small>JPG, PNG — max 10 Mo par photo</small><input type="file" name="images[]" id="images" accept="image/*" multiple style="display:none;"></div>
                ...
                <div class="s-box">
                    ...
                    @if($appartement->video)
                    <div class="vid-existing-box" id="vidExistingBox">
                        <video muted preload="metadata"><source src="{{ URL::to($appartement->video) }}" type="video/mp4"></video>
                        <div class="vex-info">
                            <div class="vex-name"><i class="fa fa-film" style="color:var(--blue);margin-right:5px;"></i>Vidéo actuelle</div>
                            <div class="vex-sub"><i class="fa fa-check-circle"></i> Vidéo enregistrée — conservée par défaut</div>
                        </div>
                        <button type="button" class="btn-vex-del" id="btnVidExDel"><i class="fa fa-trash"></i> Supprimer</button>
                    </div>
                    <input type="hidden" name="video_to_keep" id="videoToKeep" value="1">
                    @endif

                    <div class="vid-upz" id="vidUpz" {{ $appartement->video ? 'style=display:none;' : '' }}>
                        <i class="fa fa-film"></i><p>Cliquez ou glissez votre vidéo ici</p><small>MP4, MOV, AVI — max 1 min 5 sec</small>
            ...
            <div class="sl-pane" id="pane5">
                ...
                <div class="s-box">
                    <div class="s-box-title"><div class="bt-ico"><i class="fa fa-align-left"></i></div> Pourquoi choisir ce bien ? <span style="color:var(--red)">*</span></div>
                    <div class="fg">
                        <textarea name="description" id="description" placeholder="Décrivez votre bien…">{{ $appartement->description }}</textarea>
                        <div style="text-align:right;margin-top:3px;"><span id="descCt" style="font-size:11px;color:var(--ink3);">{{ strlen($appartement->description ?? '') }} / 800</span></div>
                    </div>
                </div>
            </div>
            ...
            <div class="sl-pane" id="pane6">
                ...
                <div class="s-box">
                    ...
                    <div class="fg" id="fgPrix">
                        ...
                        <div class="fi" style="max-width:260px;"><input type="text" name="prix" id="prix" value="{{ number_format($appartement->prix, 0, '', ' ') }}" placeholder="Ex: 150 000" oninput="this.value=this.value.replace(/[^0-9\s]/g,'')"><span class="fi-sfx">FCFA</span></div>
                        <span class="ferr" id="prixErr"></span>
                    </div>
                    <div class="fg" style="margin-top:14px;"><label>Prix négociable ?</label>
                        @php $negociable = $appartement->negociable ?? 'Non'; @endphp
                        <div class="yn"><button type="button" class="yn-b {{ $negociable=='Oui'?'s':'' }}" data-v="Oui" data-t="negociable">Oui</button><button type="button" class="yn-b {{ $negociable!='Oui'?'s':'' }}" data-v="Non" data-t="negociable">Non</button><input type="hidden" name="negociable" id="negociable" value="{{ $negociable }}"></div>
                    </div>
                </div>
                <div class="s-box" id="cautionBox">
                    ...
                    <div class="fg" style="max-width:220px;">
                        <select name="caution" id="caution">
                            <option value="0" {{ ($appartement->caution??'0')=='0'?'selected':'' }}>Aucune caution</option>
                            <option value="1 Mois" {{ $appartement->caution=='1 Mois'?'selected':'' }}>1 mois</option>
                            <option value="3 Mois" {{ $appartement->caution=='3 Mois'?'selected':'' }}>3 mois</option>
                            <option value="6 Mois" {{ $appartement->caution=='6 Mois'?'selected':'' }}>6 mois</option>
                            <option value="A Negocier" {{ $appartement->caution=='A Negocier'?'selected':'' }}>À négocier</option>
                        </select>
                    </div>
                </div>
            </div>
            ...
            <div class="sl-pane" id="pane7">
                ...
                <h1 class="sp-title">Vérifiez vos modifications</h1>
                <p class="sp-sub">Relisez toutes les informations avant d'enregistrer.</p>

                <div class="rc-alert"><i class="fa fa-pencil fa-lg"></i><span>Vous modifiez une annonce existante. Les nouvelles informations remplaceront les précédentes dès l'enregistrement.</span></div>
                ...
                <div class="rc-blk"><div class="rc-hd"><i class="fa fa-sliders"></i><span>Caractéristiques</span></div><div class="rc-bd">
                    ...
                    <div class="rc-row rc-terrain"><span class="rk">Clôturé</span><span class="rv" id="rc-cloture">—</span></div>
                    <div class="rc-row rc-terrain"><span class="rk">Viabilisé</span><span class="rv" id="rc-viabilise">—</span></div>
                    <div class="rc-row rc-terrain"><span class="rk">Constructible</span><span class="rv" id="rc-constructible">—</span></div>
...
<script>
var updateRoute = "{{ route('appartement.update', $appartement->id) }}";
...
(function () {
    ...
    function showSlide(n) {
        ...
        slides[n].classList.add('visible'); dots[n].classList.add('active'); cur = n;
    }
...
function setErr(el, msg) {
    el.style.borderColor = 'var(--red)'; el.style.boxShadow = '0 0 0 3px rgba(220,38,38,.07)';
    var e = el.parentNode.querySelector('.ferr-inline');
    if (!e) { e = document.createElement('span'); e.className = 'ferr-inline'; e.style.cssText = 'display:block;font-size:11px;color:var(--red);margin-top:4px;font-weight:600;'; el.parentNode.appendChild(e); }
    e.textContent = msg; el.scrollIntoView({ behavior: 'smooth', block: 'center' });
}
function clearErr(el) {
    el.style.borderColor = ''; el.style.boxShadow = '';
    var e = el.parentNode.querySelector('.ferr-inline'); if (e) e.textContent = '';
}
...
function setImageAnalysisState(state,msg){
    isAnalyzingImages=state;
    if(state){ imgErr.textContent=msg||'Analyse IA des images en cours...'; imgErr.style.display='block'; upz.style.borderColor='var(--amber)'; }
    else if(aFiles.length>0){ clearImgErrorMessage(); }
}
...
async function getMobileNetModel(){
    ...
    try{ return await mobileNetPromise; }
    catch(e){ console.warn('MobileNet indisponible',e); mobileNetLoadFailed=true; return null; }
}
...
async function validateImageWithMobileNet(file){
    ...
    try{
        ...
        var predictions=await model.classify(loaded.img);
        URL.revokeObjectURL(loaded.url);
        return evaluatePredictions(predictions||[]);
    }catch(e){
        console.warn('Analyse image ignorée',e);
        return {accepted:true, reason:''};
    }
}
getMobileNetModel();
...
var oldDep = "{{ $appartement->departement ?? '' }}";
var oldCom = "{{ $appartement->commune ?? '' }}";

Object.keys(bd).forEach(function(d){ var o=document.createElement('option'); o.value=d; o.textContent=d; if(d===oldDep) o.selected=true; dSel.appendChild(o); });

function loadCommunes(dep, sel){
    cSel.innerHTML='<option value="">Sélectionner</option>'; cSel.disabled=true;
    var a=bd[dep]; if(a&&a.length){ a.forEach(function(c){ var o=document.createElement('option'); o.value=c; o.textContent=c; if(c===sel) o.selected=true; cSel.appendChild(o); }); cSel.disabled=false; }
}
if(oldDep) loadCommunes(oldDep, oldCom);
dSel.addEventListener('change', function(){ loadCommunes(this.value,''); clearErr(dSel); });
cSel.addEventListener('change', function(){ if(this.value) clearErr(cSel); });
...
/* ── GÉO ── */
var geoLat=document.getElementById('geoLat'), geoLng=document.getElementById('geoLng');
var geoBox=document.getElementById('geoBox'), geoStatus=document.getElementById('geoStatus');
var geoAddr=document.getElementById('geoAddr'), btnGeo=document.getElementById('btnGeo');
var btnGeoReset=document.getElementById('btnGeoReset');
function showGeoStatus(t,m){ if(!geoStatus) return; geoStatus.className='geo-status '+t; geoStatus.innerHTML=m; geoStatus.style.display='flex'; }
if(btnGeo) btnGeo.addEventListener('click', function(){
    ...
    navigator.geolocation.getCurrentPosition(
        function(p){
            var lat=p.coords.latitude.toFixed(6); var lng=p.coords.longitude.toFixed(6);
            geoLat.value=lat; geoLng.value=lng; geoBox.style.display='block'; btnGeo.style.display='none';
            showGeoStatus('ok','<i class="fa fa-check-circle"></i> Position détectée.');
            fetch('https://nominatim.openstreetmap.org/reverse?lat='+lat+'&lon='+lng+'&format=json')
                .then(function(r){ return r.json(); })
                .then(function(d){ if(d&&d.display_name&&geoAddr) geoAddr.textContent='📍 '+d.display_name; })
                .catch(function(){});
        },
        function(err){ btnGeo.disabled=false; showGeoStatus('err',err.code===1?'<i class="fa fa-ban"></i> Accès refusé.':'<i class="fa fa-exclamation-circle"></i> Impossible.'); },
        { enableHighAccuracy:true, timeout:10000 }
    );
});
if(btnGeoReset) btnGeoReset.addEventListener('click', function(){
    if(geoLat) geoLat.value=''; if(geoLng) geoLng.value=''; if(geoAddr) geoAddr.textContent='';
    if(geoBox) geoBox.style.display='none'; if(btnGeo){ btnGeo.style.display=''; btnGeo.disabled=false; }
    if(geoStatus) geoStatus.style.display='none';
});

/* ── TYPE DYNAMIQUE ── */
var currentType = "{{ $appartement->type }}";
function applyTypeBlocks(){
    ...
    var cb=document.getElementById('cautionBox'); if(cb) cb.style.display=isTerrain?'none':'';
    updatePiecesTotal(); syncSurfaceTotale();
}
applyTypeBlocks();
...
function initOcGrid(gridId, onChange){
    ...
    g.querySelectorAll('.oc').forEach(function(c){
        c.addEventListener('click', function(){
            ...
            c.classList.add('s'); if(onChange) onChange(c.getAttribute('data-v'));
        });
    });
}
...
function initCompteurGrid(gridId, hiddenId){
    ...
    g.querySelectorAll('.compt-opt').forEach(function(opt){
        opt.addEventListener('click', function(){
            ...
            opt.classList.add('s');
            var inp=opt.querySelector('input[type="radio"]'); if(inp) inp.checked=true;
            var h=document.getElementById(hiddenId); if(h) h.value=opt.getAttribute('data-v');
        });
    });
}
...
document.querySelectorAll('.eq').forEach(function(item){
    item.addEventListener('click', function(){
        ...
        var inp=item.querySelector('input'); if(inp) inp.checked=item.classList.contains('s');
    });
});
...
var tL=document.getElementById('terrainLargeur'); var tLo=document.getElementById('terrainLongueur');
...
function syncSurfaceTotale(){
    var h=document.getElementById('surfaceTotaleHidden'); if(!h) return;
    var map={'Maison':'surface','Appartement':'surface','Terrain':'surfaceTerrain','Bureaux':'surfaceBureau','Boutique':'surfaceBoutique'};
    var id=map[currentType]; if(!id) return;
    var el=document.getElementById(id); h.value=(el&&el.value)?el.value:'0';
}
function updateSurfaceTotale(){
    ...
    var t=s+c+ch+b;
    var surfEl=document.getElementById('surface'); if(surfEl&&t>0) surfEl.value=t;
    syncSurfaceTotale();
}
['surfSalon','surfCuisine','surfChambre','surfSdb'].forEach(function(id){ var el=document.getElementById(id); if(el) el.addEventListener('input',updateSurfaceTotale); });
var surfEl=document.getElementById('surface'); if(surfEl) surfEl.addEventListener('input',syncSurfaceTotale);
var surfBureau=document.getElementById('surfaceBureau'); if(surfBureau) surfBureau.addEventListener('input',syncSurfaceTotale);
var surfBoutique=document.getElementById('surfaceBoutique'); if(surfBoutique) surfBoutique.addEventListener('input',syncSurfaceTotale);
var surfTerrain=document.getElementById('surfaceTerrain'); if(surfTerrain) surfTerrain.addEventListener('input',syncSurfaceTotale);
...
function updatePiecesTotal(){
    ...
    Object.keys(cards).forEach(function(fid){ var inp=document.getElementById(fid); var card=document.getElementById(cards[fid]); if(inp&&card) card.classList.toggle('has-value',parseInt(inp.value)>0); });
}
document.querySelectorAll('.cb').forEach(function(b){
    b.addEventListener('click', function(){
        var tid=b.getAttribute('data-t'); var el=document.getElementById(tid); if(!el) return;
        var v=parseInt(el.value)||0; var mn=parseInt(el.min)||0;
        if(b.getAttribute('data-a')==='+') v++; else if(v>mn) v--;
        el.value=v;
...
document.querySelectorAll('.yn-b[data-t]').forEach(function(b){
    b.addEventListener('click', function(){
        ...
        b.classList.add('s');
        var el=document.getElementById(b.getAttribute('data-t')); if(el) el.value=b.getAttribute('data-v');
    });
});

/* ── PHOTOS EXISTANTES ── */
var existingList = "{{ $appartement->images ?? '' }}".split('|').filter(Boolean);
var deletedImgs  = [];
var keepInput    = document.getElementById('imagesToKeep');

window.toggleExisting = function(idx){
    var el = document.getElementById('exph-'+idx);
    if(!el) return;
    var img = el.getAttribute('data-img');
    if(el.classList.contains('deleted')){
        el.classList.remove('deleted');
        deletedImgs = deletedImgs.filter(function(x){ return x!==img; });
    } else {
        el.classList.add('deleted');
        deletedImgs.push(img);
    }
    var kept = existingList.filter(function(i){ return deletedImgs.indexOf(i)===-1; });
    if(keepInput) keepInput.value = kept.join('|');
};

/* ── UPLOAD NOUVELLES PHOTOS ── */
var upz=document.getElementById('upz'), fInp=document.getElementById('images');
var prG=document.getElementById('prevG'), imgErr=document.getElementById('imgErr');
...
async function addF(files){
    ...
    var refused=[];
    setImageAnalysisState(true,'Analyse IA des images en cours...');
    for(var i=0;i<incoming.length;i++){
        ...
        r.onload=function(ev){
            ...
            dl.addEventListener('click',function(){ var idx=aFiles.indexOf(fileRef); if(idx!==-1) aFiles.splice(idx,1); d.remove(); if(aFiles.length===0) clearImgErrorMessage(); });
...
/* ── VIDÉO EXISTANTE ── */
var btnVidExDel  = document.getElementById('btnVidExDel');
var vidExistingBox = document.getElementById('vidExistingBox');
var videoToKeep  = document.getElementById('videoToKeep');
var vidUpz       = document.getElementById('vidUpz');
if(btnVidExDel){
    btnVidExDel.addEventListener('click', function(){
        if(vidExistingBox) vidExistingBox.style.display='none';
        if(videoToKeep)    videoToKeep.value='0';
        if(vidUpz)         vidUpz.style.display='';
    });
}

/* ── UPLOAD NOUVELLE VIDÉO ── */
var vidInput=document.getElementById('videoInput');
var vidPreview=document.getElementById('vidPreview'), vidPlayer=document.getElementById('vidPlayer');
var vidName=document.getElementById('vidName'), vidErr=document.getElementById('vidErr');
var vidDel=document.getElementById('vidDel'), vidFile=null;
...
if(vidInput) vidInput.addEventListener('change',function(){ if(this.files.length) handleVideo(this.files[0]); this.value=''; });
function handleVideo(f){
    if(vidErr) vidErr.style.display='none';
    if(!f.type.startsWith('video/')){ if(vidErr){ vidErr.textContent='Ce n\'est pas une vidéo.'; vidErr.style.display='block'; } return; }
    if(f.size>MAX_VID){ if(vidErr){ vidErr.textContent='La vidéo dépasse 100 Mo.'; vidErr.style.display='block'; } return; }
    var url=URL.createObjectURL(f);
    var tmp=document.createElement('video'); tmp.preload='metadata'; tmp.src=url;
    tmp.onloadedmetadata=function(){
        URL.revokeObjectURL(url);
        if(tmp.duration>MAX_VID_DUR){ if(vidErr){ vidErr.textContent='Durée dépassée (max 1 min 5 sec).'; vidErr.style.display='block'; } vidFile=null; return; }
        vidFile=f; if(vidPlayer) vidPlayer.src=URL.createObjectURL(f);
        if(vidName) vidName.textContent=f.name+' ('+Math.round(f.size/1024/1024*10)/10+' Mo)';
        if(vidPreview) vidPreview.style.display='block';
        if(vidUpz)    vidUpz.style.display='none';
    };
}
if(vidDel) vidDel.addEventListener('click',function(){ vidFile=null; if(vidPlayer) vidPlayer.src=''; if(vidPreview) vidPreview.style.display='none'; if(vidUpz) vidUpz.style.display=''; });

/* ── DESCRIPTION ── */
var dTa=document.getElementById('description'), dCt=document.getElementById('descCt');
if(dTa) dTa.addEventListener('input',function(){
    var len=this.value.length; dCt.textContent=len+' / 800';
    dCt.style.color=len>700?'var(--red)':'var(--ink3)';
    if(len>=10) clearErr(this);
});

if(document.getElementById('prix')) document.getElementById('prix').addEventListener('input',function(){
    var p=this.value.replace(/\s/g,'');
    if(p&&!isNaN(parseFloat(p))&&parseFloat(p)>0){ clearErr(this); document.getElementById('prixErr').style.display='none'; document.getElementById('fgPrix').classList.remove('err'); }
});
...
function updateMobileProgress(n){
    var lbl=document.getElementById('mpStepLabel'); var num=document.getElementById('mpStepNum'); var fill=document.getElementById('mpFill');
...
function goTo(n){
    ...
    if(n===tot){ nBtn.innerHTML='<i class="fa fa-check" style="margin-right:6px;"></i>Enregistrer les modifications'; nBtn.style.background='var(--gr)'; }
    else{ nBtn.innerHTML='Suivant <i class="fa fa-arrow-right" style="margin-left:6px;"></i>'; nBtn.style.background='var(--gr)'; }
    currentStep=n; updateMobileProgress(n); window.scrollTo({top:0,behavior:'smooth'});
    if(n===7) fillRecap();
}
document.getElementById('btnNext').addEventListener('click',function(){ if(!validate(currentStep)) return; if(currentStep<tot) goTo(currentStep+1); else doSubmit(); });
document.getElementById('btnPrev').addEventListener('click',function(){ if(currentStep>1) goTo(currentStep-1); });
...
function validate(n){
    ...
    if(n===3){
        if(currentType==='Maison'||currentType==='Appartement'){ var s=document.getElementById('surface'); if(!s.value||parseInt(s.value)<=0){ setErr(s,'Indiquez la surface totale.'); return false; } else clearErr(s); }
        if(currentType==='Terrain'){ var st=document.getElementById('surfaceTerrain'); if(!st.value||parseInt(st.value)<=0){ setErr(st,'Indiquez la superficie.'); return false; } else clearErr(st); }
        if(currentType==='Bureaux'){ var sb=document.getElementById('surfaceBureau'); if(!sb.value||parseInt(sb.value)<=0){ setErr(sb,'Indiquez la surface.'); return false; } else clearErr(sb); }
        if(currentType==='Boutique'){ var sbo=document.getElementById('surfaceBoutique'); if(!sbo.value||parseInt(sbo.value)<=0){ setErr(sbo,'Indiquez la surface.'); return false; } else clearErr(sbo); }
        syncSurfaceTotale(); return true;
    }
    if(n===4){ if(isAnalyzingImages){ setImgErrorMessage('Analyse IA des images en cours...'); upz.scrollIntoView({behavior:'smooth',block:'center'}); return false; } return true; }
    if(n===5){ if(dTa.value.trim().length<10){ setErr(dTa,'La description doit comporter au moins 10 caractères.'); return false; } clearErr(dTa); return true; }
    if(n===6){ var pEl=document.getElementById('prix'); var pv=pEl.value.replace(/\s/g,''); if(!pv||isNaN(parseFloat(pv))||parseFloat(pv)<=0){ setErr(pEl,'Prix invalide.'); document.getElementById('prixErr').style.display='block'; document.getElementById('fgPrix').classList.add('err'); return false; } clearErr(pEl); document.getElementById('prixErr').style.display='none'; document.getElementById('fgPrix').classList.remove('err'); return true; }
    return true;
}
...
function fillRecap(){
    ...
    var lat=geoLat?geoLat.value:''; var lng=geoLng?geoLng.value:'';
    ...
    if(isTerrain){
        ...
        document.getElementById('rc-cloture').textContent=g('cloture')||'—';
        document.getElementById('rc-viabilise').textContent=g('viabilise')||'—';
        document.getElementById('rc-constructible').textContent=g('constructible')||'—';
    }
    ...
    var eqList=[];
    document.querySelectorAll('.eq.s').forEach(function(e){ var par=e.closest('.type-block'); if(par&&par.classList.contains('d-none')) return; var sp=e.querySelector('span'); if(sp) eqList.push(sp.textContent.trim()); });
...
function doSubmit(){
    if(isAnalyzingImages){
        setImgErrorMessage('Analyse IA des images en cours...');
        goTo(4);
    ...
    formData.delete('images[]'); aFiles.forEach(function(f){ formData.append('images[]',f); });
    formData.delete('video'); if(vidFile) formData.append('video',vidFile);
    var nBtn=document.getElementById('btnNext');
    nBtn.innerHTML='<i class="fa fa-spinner fa-spin" style="margin-right:6px;"></i>Enregistrement…'; nBtn.disabled=true;
    $.ajax({
        url:updateRoute, method:'POST', data:formData, contentType:false, processData:false,
        headers:{'X-CSRF-TOKEN':$('meta[name="csrf-token"]').attr('content')},
        success:function(res){
            var status=parseInt(res.status??res);
            if(status===200||status===201){
                if(typeof showToast==='function') showToast('success','Annonce modifiée avec succès');
                setTimeout(function(){ window.location.href=retourRoute; },3000);
            } else {
                alert('Erreur. Veuillez réessayer.');
                nBtn.innerHTML='<i class="fa fa-check" style="margin-right:6px;"></i>Enregistrer les modifications'; nBtn.disabled=false;
            }
        },
        error:function(xhr){
            ...
            if(imageError){
                imgErr.textContent=imageError;
                imgErr.style.display='block';
                upz.style.borderColor='var(--red)';
                goTo(4);
                upz.scrollIntoView({behavior:'smooth',block:'center'});
                nBtn.innerHTML='<i class="fa fa-check" style="margin-right:6px;"></i>Enregistrer les modifications';
                nBtn.disabled=false;
                return;
            }
